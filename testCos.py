...
# 提取所有测试集的特征，然后划分support 和 query 计算余弦相似度
def feature_evaluation(cl_data_file, n_way=5, n_support=5, n_query=15, adaptation=False):
    class_list = cl_data_file.keys()
    #print("class_list",class_list)
    select_class = random.sample(class_list, n_way)
    #print("select_classes",select_class)
    z_all = []  #[[cls img_feat][...][...]]
    for cl in select_class:
        img_feat = cl_data_file[cl]  #list 600 element
        # print('len:',len(img_feat))
        # print("feat shape",img_feat[0].shape)  #1152
        perm_ids = np.random.permutation(len(img_feat)).tolist() #产生一个随机排列列表
        z_all.append([np.squeeze(img_feat[perm_ids[i]]) for i in range(n_support + n_query)])  # stack each batch

    z_all = torch.from_numpy(np.array(z_all))
    #print(z_all.shape)  #(5,600,1152)
    ...
    z_support, z_query = parse_feature(z_all, n_support)# 切分([5, 5, 1152]) ([5, 595, 1152])
    #print("support shape ",z_support.shape,"query shape",z_query.shape)
    z_support = z_support.contiguous().view(n_way * n_support, -1)
    z_query = z_query.contiguous().view(n_way * n_query, -1)
    cos_score = cos_sim(z_support, z_query)# [25, 2975]
    #print("cos_score",cos_score.shape)
