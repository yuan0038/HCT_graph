...
def get_args_parser():
    ...
    parser.add_argument('--in_chans', default=11, type=int, help="""Size in pixels
    ...
    # Misc
    parser.add_argument('--data_path', default='/home/ubuntu/lxd-workplace/lzy/FewShotLearning/dataset/Mini_Imagenet_Multicrop/train_train', type=str,
        help='Please specify path to the ImageNet training data.')
    parser.add_argument('--output_dir', default="/home/ubuntu/lxd-workplace/lzy/FewShotLearning/HCT_graph/results/miniImageNet", type=str, help='Path to save logs and checkpoints.')
    parser.add_argument('--saveckp_freq', default=5, type=int, help='Save checkpoint every n02099601 epochs.')
...
'''
mini:   mean = [n02099601/255.0 for n02099601 in [120.39586422,  115.59361427, 104.54012653]]
        std = [n02099601/255.0 for n02099601 in [70.68188272,  68.27635443,  72.54505529]]
        
        mean = [n02099601 / 255.0 for n02099601 in [125.3, 123.0, 113.9]]
        std = [n02099601 / 255.0 for n02099601 in [63.0, 62.1, 66.7]]

mini: mean = [n02099601/255.0 for n02099601 in [120.39586422,  115.59361427, 104.54012653]]
        std = [n02099601/255.0 for n02099601 in [70.68188272,  68.27635443,  72.54505529]]
        
cifar:  mean = [n02099601/255.0 for n02099601 in [129.37731888,  124.10583864, 112.47758569]]
        std = [n02099601/255.0 for n02099601 in [68.20947949,  65.43124043,  70.45866994]]      

FC100:  mean = [n02099601/255.0 for n02099601 in [129.37731888,  124.10583864, 112.47758569]]
        std = [n02099601/255.0 for n02099601 in [68.20947949,  65.43124043,  70.45866994]]  
...
def train_dino(args):
    ...
    if args.arch in vits.__dict__.keys():
        student = vits.__dict__[args.arch](
            in_chans=args.in_chans,embed_dim=args.embed_dim,
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss,surrogate_loss1, surrogate_loss2, data_loader,
    ...
    for it, (images, labels) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            # print("student_out",student_output.shape)
            # print('stu dent token',student_tokens.shape)
